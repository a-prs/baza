...
## Шаг 8: Базовая безопасность сервера

Сервер запущен — теперь закроем лишние двери.

### Настрой firewall (ufw)

По умолчанию на сервере открыты все порты. Лучше явно указать, что разрешено:

```bash
apt install ufw -y

ufw allow ssh        # SSH (порт 22) — чтобы не потерять доступ
ufw allow 80         # HTTP
ufw allow 443        # HTTPS
...
::: warning Сначала разреши SSH!
Если включить `ufw` не разрешив SSH (`ufw allow ssh`) — потеряешь доступ к серверу. Команды выше написаны в правильном порядке — следуй им.
:::

### Создай обычного пользователя

Работать под `root` — небезопасно. Одна ошибка в команде может снести систему. Заведи отдельного пользователя для повседневной работы:

```bash
adduser myuser               # создать пользователя (придумай имя)
usermod -aG sudo myuser      # дать права sudo
```

Теперь можно подключаться через Termius под `myuser` вместо `root`. Когда нужны права администратора — используй `sudo` перед командой.
...
✅ Настроили firewall и создали безопасного пользователя

Результат: mysite.com — твой сайт в интернете, сервер защищён
```
